...
import Contact from './commpontes/contact/Contact';
...
const App = () => {
  ...
  //   return cart.find((product) => product.id ===id)?.qty || 0
  ...
  //  const cartqunty =cart.reduce((qty , x ) =>(
  //   (qty , x) => x.qty + qty
  //    ), 0)
    ...
    const HeartQuantity = heart.reduce((qty, x) => {
      return qty + x.qty;
    }, 0);
  return (
    ...
    <>
    <BrowserRouter>
    <Navbars cart={cart} setcart={setcart} cartqunty={cartQuantity} HeartQuantity={HeartQuantity}/>
    <Routes>
       ...
       <Route path='/heart' element={<Heart heart={heart} setheart={setheart} addtocart={addtocart} HeartQuantity={HeartQuantity}/>} />
